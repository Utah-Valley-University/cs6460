...
This simulation visualizes Breadth-First Search (BFS) using a water maze,
similar to the CS 188 Berkeley videos. Water spreads through the maze
following BFS exploration order (level by level).
...
class WaterMaze:
    """Represents a maze where water spreads using BFS."""
    ...
    def __init__(self, width=15, height=15, start=(1, 1), goal=None):
        ...
        # BFS tracking - use queue (FIFO) instead of stack
        self.queue = deque([start])
        self.visited = {start}
        self.exploration_order = []
        self.found_goal = False
        self.distances = {start: 0}  # Track distance from start
    ...
    def bfs_step(self):
        ...
        for current in nodes_at_level:
            ...
            # All neighbors will be explored together at next level (next time step)
            for next_pos in neighbors:
                ...
                # Time will be set when they're actually explored at next level
...
class WaterMazeVisualizer:
    """Visualizes the BFS water maze animation."""
...
def main():
    ...
    print("BFS Water Maze Simulation")
    print("=" * 50)
    print("This simulation shows how Breadth-First Search explores a maze")
    print("by visualizing water spreading through the maze level by level.")
    print("\nLegend:")
    print("  Green square (S) = Start")
    print("  Red square (G) = Goal")
    print("  Blue circles = Water spreading (BFS exploration)")
    print("  Light blue = Previously explored cells")
    print("  Yellow circles = Current level being explored (queue)")
