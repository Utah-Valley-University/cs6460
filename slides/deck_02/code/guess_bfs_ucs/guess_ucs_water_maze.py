#!/usr/bin/env python3
"""
UCS Water Maze Simulation with Weighted Edges (Shallow/Deep Water)

This simulation visualizes Uniform Cost Search (UCS) on a maze with weighted edges:
- Shallow water (cost 1) - light blue
- Deep water (cost 2) - dark blue

UCS CONSIDERS the weights and explores by cost contours (shallow water first, then deep).
This contrasts with BFS which ignores weights and spreads by number of steps.

This is for slide 49: "Guess Which of the Three - DFS, BFS, UCS?"
...
class WeightedWaterMazeUCS:
    """Represents a maze with weighted edges (shallow/deep water) where UCS considers weights."""
    ...
    def __init__(self, width=15, height=15, start=(1, 1), goal=None):
        ...
        # UCS tracking - use priority queue (min-heap)
        # UCS CONSIDERS edge costs - explores by cumulative cost
        # Priority queue: (cumulative_cost, time_added, position)
        self.priority_queue = [(0, 0, start)]  # (cost, tiebreaker, position)
        self.visited = {start}
        self.exploration_order = []
        self.found_goal = False
        self.costs = {start: 0}  # Cumulative cost from start
        self.tiebreaker = 0  # For breaking ties in priority queue
    ...
    def ucs_step(self):
        """
        Perform one step of UCS exploration - explore node with lowest cumulative cost.
        UCS CONSIDERS edge costs - explores by cost contours.
        """
        if not self.priority_queue or self.found_goal:
            return False
        
        # Get node with lowest cumulative cost
        current_cost, _, current = heapq.heappop(self.priority_queue)
        
        # Skip if already explored (may have been added multiple times with different costs)
        if self.water_state[current[0], current[1]] == 2:
            return True
        ...
        # UCS: add neighbors to priority queue with cumulative cost
        for next_pos in neighbors:
            # Calculate edge cost
            edge_cost = self.get_edge_cost(current, next_pos)
            new_cost = current_cost + edge_cost
            
            # Only add if we found a better path or haven't visited yet
            if next_pos not in self.costs or new_cost < self.costs[next_pos]:
                self.costs[next_pos] = new_cost
                self.tiebreaker += 1
                heapq.heappush(self.priority_queue, (new_cost, self.tiebreaker, next_pos))
                
                # Mark as in queue if not already explored
                if self.water_state[next_pos[0], next_pos[1]] == 0:
                    self.water_state[next_pos[0], next_pos[1]] = 1  # Wet (in queue)
        
        # Increment time after processing node
        self.current_time += 1
        return True
    ...
    def get_current_cost_level(self):
        """Get the current cost level being explored (for visualization)."""
        if self.priority_queue:
            return self.priority_queue[0][0]  # Cost of next node to explore
        return 0


class WeightedWaterMazeVisualizerUCS:
    """Visualizes the UCS water maze animation with weighted edges."""
    
    def __init__(self, maze: WeightedWaterMazeUCS, speed=100):
        """
        Initialize the visualizer.
        
        Args:
            maze: WeightedWaterMazeUCS instance
        ...
        self.ax.set_title('Water Maze Simulation - Guess: DFS, BFS, or UCS?', 
    ...
    def update_display(self):
        ...
        # Group explored cells by cost for better visualization
        cost_groups = {}
        for i in range(self.maze.height):
            for j in range(self.maze.width):
                if self.maze.maze[i, j] != 1:  # Not a wall
                    pos = (i, j)
                    if pos in self.maze.costs:
                        cost = self.maze.costs[pos]
                        if cost not in cost_groups:
                            cost_groups[cost] = []
                        cost_groups[cost].append((i, j))
            ...
            for j in range(self.maze.width):
                if self.maze.maze[i, j] != 1:  # Not a wall
                    ...
                    if state > 0:  # Wet or explored
                        # For UCS, color by cost contour instead of time
                        if pos in self.maze.costs:
                            cost = self.maze.costs[pos]
                            # Normalize cost to 0-1 range (assuming max cost around 30)
                            max_cost = max(cost_groups.keys()) if cost_groups else 1
                            cost_ratio = cost / max(max_cost, 1)
                            cost_ratio = max(0.0, min(1.0, cost_ratio))
                            
                            # Bright blue for low cost (shallow), darker for high cost (deep)
                            r = int(0)
                            g = int(102 - 82 * cost_ratio)  # 102 -> 20
                            b = int(255 - 175 * cost_ratio)  # 255 -> 80
                            ...
                            # Default color for nodes in queue
                            color = '#0066ff'
                            edge_color = '#004488'
        ...
        # Add info text
        current_cost = self.maze.get_current_cost_level()
        info = f"Explored: {len([p for p in self.maze.costs.keys() if self.maze.water_state[p[0], p[1]] == 2])} cells | "
        info += f"Queue size: {len(self.maze.priority_queue)} | "
        info += f"Current cost level: {current_cost}"
        if self.maze.found_goal:
            info += " | GOAL FOUND!"
            if self.maze.goal in self.maze.costs:
                info += f" (Cost: {self.maze.costs[self.maze.goal]})"
        self.ax.text(0.5, -0.3, info, transform=self.ax.transAxes,
                    fontsize=10, ha='left', va='top')
        
        # Add hint
        hint = "Hint: Does water spread by number of steps, depth-first, or by cost?"
...
def main():
    """Main function to run the UCS water maze simulation."""
    print("=" * 60)
    print("Water Maze Simulation - Guess: DFS, BFS, or UCS?")
    print("=" * 60)
    print("\nThis maze has:")
    print("  - Shallow water (cost 1) - light blue background")
    print("  - Deep water (cost 2) - dark blue background")
    print("\nWatch how the water spreads:")
    print("  - Does it spread in concentric circles (by steps)? → BFS")
    print("  - Does it go deep first then backtrack? → DFS")
    print("  - Does it spread by cost (shallow first, then deep)? → UCS")
    print("\nLegend:")
    ...
    # Create maze
    maze = WeightedWaterMazeUCS(width=15, height=15, start=(1, 1), goal=(13, 13))
    
    # Create visualizer
    visualizer = WeightedWaterMazeVisualizerUCS(maze, speed=150)
